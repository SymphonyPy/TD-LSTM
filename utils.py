...
def change_y_to_onehot(y):
    ...
    y_onehot_mapping = dict(list(zip(class_set, list(range(n_class)))))
    onehot = []
    for label in y:
        tmp = [0] * n_class
...
def load_inputs_twitter(input_file, word_id_file, sentence_len, type_='', encoding='utf8'):
    ...
    for i in range(0, len(lines), 3):
        ...
        target_word = [word_to_id.get(w, 0) for w in target_word]
...
def extract_aspect_to_id(input_file, aspect2id_file):
    ...
    aspect2id = list(zip(targets, list(range(1, len(lines) + 1))))
